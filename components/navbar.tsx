				...
				<div className='flex h-16 items-center justify-between'>
					<div className='flex items-center space-x-4'>
						<Link href='/users' className='flex items-center space-x-2'>
							<span className='hidden font-bold text-xl text-gray-900 sm:inline-block'>TIPAS</span>
						</Link>
									...
									<div className='border-t pt-4'>
										<div className='flex items-center space-x-3 px-3 py-2'>
											<Avatar className='h-10 w-10'>
												<AvatarImage src={user?.profile_image || "/placeholder.svg"} alt={`${user?.first_name} ${user?.last_name}`} />
												<AvatarFallback>
													{user?.first_name
														.split(" ")
														.map((n) => n[0])
